...
/** Edge a veces lee "?" suelto como la letra J; español usa ¿…? */
function fixSpanishPunctuation(text) {
  if (/^[\s?!.…,;:]+$/u.test(text)) {
    if (text.includes("?")) return "signo de interrogación";
    if (text.includes("!")) return "signo de exclamación";
    return "";
  }
  ...
  // "dijo ?" o "dijo  ?" — interrogante suelto sin palabra
  s = s.replace(/\s+\?(?=\s|$)/g, "");
  s = s.replace(/\s+\?(?=[,.!])/g, "");
...
export function prepareTextForSpeech(text) {
  let s = stripDiscordMarkup(String(text ?? ""));
  s = normalizeUnicodePunctuation(s);
  s = removeIncompatibleCharacters(s);
  s = fixSpanishPunctuation(s);
  return s.trim();
}

function localeFromVoice(voice) {
  const match = /^([a-z]{2}-[A-Z]{2})/i.exec(voice);
  return match?.[1] ?? "es-ES";
}
...
export function voiceListHeaders() {
  return {
    ...wsHeaders(),
...
export async function fetchEdgeVoiceCatalog() {
  const resp = await fetch(VOICE_LIST_URL, { headers: voiceListHeaders() });
  if (!resp.ok) {
    throw new Error(`Edge voice list HTTP ${resp.status}`);
  }
  const data = await resp.json();
  if (!Array.isArray(data)) {
    throw new Error("Edge voice list inválida");
  }
  return data;
}
